...
<<<<<<< HEAD
  function load() {
    ...
    const content_area =
      $(temp_a4).height() - 0.25 * parseFloat($(temp_a4).css("padding-bottom"));
      ...
      if ($(elements[i]).hasClass("page-break")) {
        ...
        if ($(new_element).is("ul,ol,p,div")) {
          ...
          let remaining = Math.floor(
            (content_area - container.height()) / line_height
          );
          if (remaining <= 0) {
            [temp_a4, container] = new_page(temp_a4);
            container.append($(new_element));
            ...
            container.append($(new_element));
            let line_start = 0;
            let line_end = Math.ceil(
              $(new_element).outerHeight(true) / line_height
            );
            ...
            while (line_start < line_end) {
              ...
              bottom_part.css(
                "max-height",
                Math.ceil(line_start + line_per_page) * line_height + "px"
              );
              let printed_line = Math.ceil(
                $(bottom_part).outerHeight(true) / line_height
              );
...
<<<<<<< HEAD
  const render_html = function () {
    ...
    let font_waiting =
      rendered.css("font-weight") +
      " " +
      rendered.css("font-size") +
      " " +
